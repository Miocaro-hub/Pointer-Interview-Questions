...
int main()
{
	...
	printf("%d\n", sizeof(a+1)); //4 - 表示第二行的地址,a是首元素地址,而二维数组的首元素是第一行
	printf("%d\n", sizeof(*(a+1))); //16 - 第二行数组解引用,有4个整型元素
	printf("%d\n", sizeof(&a[0]+1)); //4 - 表示第二行的地址
	printf("%d\n", sizeof(*(&a[0] + 1))); //16 - 第二行一维数组解引用
	printf("%d\n", sizeof(*a)); //16 - a表示首元素地址(第一行地址),*a表示第一行
	printf("%d\n", sizeof(a[3])); //16 - 虽然这个数组只有3行,a[3]不会真的访问第四行,而是只是一个计算类型大小,和a[0]一个道理
	printf("\n");
